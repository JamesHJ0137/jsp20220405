...
SELECT * FROM Customers LIMIT 3; -- 조회결과 중 첫번째 3개의 레코드만
...
-- Product 테이블 조회해서 가장 가격이 높은 상품 3개 조회
SELECT * FROM Products ORDER BY Price DESC LIMIT 3;

-- LIMIT [부터], [몇 개]
-- Product 테이블 조회해서 두번째로 가격이 비싼 상품부터 네번째로 가격이 비싼 상품 조회 
...
-- 시작 위치 => (페이지 번호 -1) * 한 페이지당 몇 개
-- example) 4페이지 = 3 X 10 = 30, therefore, (30, 10)